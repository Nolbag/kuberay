...
func (r *RayClusterReconciler) reconcileIngress(ctx context.Context, instance *rayv1.RayCluster) error {
	...
	if instance.Spec.HeadGroupSpec.EnableIngress == nil || !*instance.Spec.HeadGroupSpec.EnableIngress {
		// Keep the downstream safety contract: disabling direct exposure also
		// removes any previously-created Route or Ingress.
...
func (r *RayClusterReconciler) cleanupIngress(ctx context.Context, instance *rayv1.RayCluster) error {
	...
	if r.options.IsOpenShift {
		...
		if err := r.List(ctx, &routes, filterLabels...); err != nil {
			return err
		}
		for i := range routes.Items {
			...
			logger.Info("Deleted Route because enableIngress is false", "route", route.Name)
		}
	}

	ingresses := networkingv1.IngressList{}
	if err := r.List(ctx, &ingresses, filterLabels...); err != nil {
		return err
	}
	for i := range ingresses.Items {
		...
		logger.Info("Deleted Ingress because enableIngress is false", "ingress", ingress.Name)
	}
	return nil
}
...
func (r *RayClusterReconciler) createHeadPod(ctx context.Context, instance rayv1.RayCluster, clusterHash string) error {
	...
	// Wait for the downstream authentication controller to finish creating its
	// ServiceAccount and HTTPRoute before creating a head pod with the proxy
	// sidecar. This preserves the existing OpenShift startup ordering contract.
	...
	if shouldEnableAuth {
		if features.Enabled(features.RayClusterStatusConditions) {
			authReadyCondition := meta.FindStatusCondition(instance.Status.Conditions, string(rayv1.AuthenticationReady))
			...
			if conditionNotReady || conditionStale {
				...
				if authReadyCondition != nil {
					...
					if conditionStale {
						message = fmt.Sprintf("Condition is stale (observedGeneration=%d, currentGeneration=%d): %s", authReadyCondition.ObservedGeneration, instance.Generation, message)
					}
				}
			...
			if err := r.Get(ctx, client.ObjectKey{Name: namer.ServiceAccountName(authMode), Namespace: instance.Namespace}, sa); err != nil {
				if errors.IsNotFound(err) {
					return fmt.Errorf("waiting for ServiceAccount %s to be created for authentication", namer.ServiceAccountName(authMode))
				}
				return err
			}
		}
...
func (r *RayClusterReconciler) buildHeadPod(ctx context.Context, instance rayv1.RayCluster) corev1.Pod {
	...
	// OpenShift Gateway exposure is backed by the downstream authentication
	// controller. Inject its proxy only when the security annotation enables it;
	// upstream RayCluster authOptions remains independent of this platform path.
	authMode := utils.DetectAuthenticationMode(r.options.IsOpenShift)
	if utils.ShouldEnableOAuth(&instance, authMode) || utils.ShouldEnableOIDC(&instance, authMode) {
		namer := utils.NewResourceNamer(&instance)
		utils.InjectAuthSidecar(&podConf.Spec, &instance, authMode, GetOIDCProxySidecar, GetOIDCProxyVolumes, namer.ServiceAccountName(authMode))
	}
